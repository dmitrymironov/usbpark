...
static IBaseFilter *
FindCaptureDevice( std::string *p_devicename,
                   std::list<std::string> *p_listdevices, bool b_audio )
{
    ...
    if( FAILED(hr) )
    {
        std::cerr << "failed to create the device enumerator (0x" << std::hex << hr << std::dec << ")" << std::endl;
        return NULL;
    }
    ...
    if( FAILED(hr) )
    {
        std::cerr << "failed to create the class enumerator (0x" << std::hex << hr << std::dec << ")" << std::endl;
        return NULL;
    }
    ...
    {
        std::cerr <<  "no " << ( b_audio ? "audio" : "video" ) << " capture device was detected" << std::endl;
        return NULL;
    }
    ...
    while( p_class_enum->Next( 1, &p_moniker, &i_fetched ) == S_OK )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            ...
            if( SUCCEEDED(hr) )
            {
                ...
                std::string devname = std::string(p_buf);
                free( p_buf) ;
                ...
                {
                    std::cout << "asked for " << p_devicename->c_str() << ", binding to " << devname.c_str()  << std::endl;
                    /* NULL possibly means we don't need BindMoniker BindCtx ?? */
                    hr = p_moniker->BindToObject( NULL, 0, IID_IBaseFilter,
                                                  (void **)&p_base_filter );
                    if( FAILED(hr) )
                    {
                        std::cerr << "couldn't bind moniker to filter object (0x" << std::hex << hr << std::dec << ")" << std::endl;
                        p_moniker->Release();
...
std::list<std::string> Novorado::Parking::WebCamTab::getCaptureDevices()
{
	...
	bool b_audio=false;
	FindCaptureDevice(NULL,&rv,b_audio);
	return rv;
}
